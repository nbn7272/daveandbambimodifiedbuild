...
/// Code created by totally not rozebud for FPS Plus (thanks totally me)
// modified by MINUSCLUBMINUSCLUB for use in dave engine/fart
...
class KeyBindMenu extends FlxSubState
{
	...
	function addKey(r:String){
        ...
        if(shouldReturn){
            keys[curSelected] = r;
           //FlxG.sound.play(Paths.sound('Hover','clown')); //tricky sound
        }
        else{
            keys[curSelected] = tempKey;
            //FlxG.sound.play(Paths.sound('confirm','clown')); //tricky sound
            keyWarning.alpha = 1;
            warningTween.cancel();
