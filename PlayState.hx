...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		SONG.song
		+ " "
		+ (curSong.toLowerCase() != 'splitathon' ? (storyDifficulty == 3 && SONG.song.toLowerCase() != "very-screwed" ? "Legacy" : storyDifficulty == 3 && SONG.song.toLowerCase() == "very-screwed" ? "EXTREME" : storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy") : "Finale"), 16);
	...
	function createBackgroundSprites(song:String):FlxTypedGroup<FlxSprite>
	{
		...
		switch (song)
		{
			...
			case 'bonus-song' | 'glitch' | 'secret':
				...
				UsingNewCam = true;
			case 'polygonized' | 'furiosity' | 'cheating' | 'unfairness' | 'cheating-high-pitched' | 'unfairness-high-pitched' | 'torture' | 'disruption':
				defaultCamZoom = 0.9;
	...
	override public function update(elapsed:Float)
	{
		...
		if (SONG.song.toLowerCase() == 'oppositionn') // no
			{
				playerStrums.forEach(function(spr:FlxSprite)
				{
		...
		if (SONG.song.toLowerCase() == 'thearchyy') // no
			{
				playerStrums.forEach(function(spr:FlxSprite)
				{
					spr.x = ((FlxG.width / 12) - (spr.width / 7)) + (Math.sin(elapsedtime * 4 + (spr.ID)) * FlxG.random.int(500, 520));
		...
		if (SONG.song.toLowerCase() == 'phonophobias' && curBeat > 10) // no
			{
				dadStrums.forEach(function(spr:FlxSprite)
				{
					if (!swag) spr.y += 1;
